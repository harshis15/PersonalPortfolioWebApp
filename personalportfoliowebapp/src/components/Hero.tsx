  ...
  return (
    <section id="hero" className="hero-mesh relative min-h-screen overflow-hidden px-5 pb-16 pt-28 md:px-8 md:pt-32">
      ...
      <div className="relative mx-auto grid w-full max-w-6xl items-center gap-12 lg:grid-cols-[1.45fr_1fr]">
        ...
        >
          <motion.div variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }}>
            <span className="inline-flex items-center gap-2 rounded-full border border-accent-border bg-raised px-4 py-1.5 text-xs font-mono uppercase tracking-[0.14em] text-mint">
              <span className="h-2 w-2 animate-pulse rounded-full bg-mint" />
              {hero.availability}
            </span>
          </motion.div>

          <motion.p variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="text-sm uppercase tracking-[0.2em] text-label-muted">
            {brand.roleLine}
          </motion.p>
          ...
          <motion.p variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="max-w-2xl text-base leading-relaxed text-cream md:text-lg">
          ...
          <motion.div variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="flex flex-wrap items-center gap-4">
            ...
            <a
              href={resumeUrl}
              className="inline-flex items-center rounded-full border border-accent-border bg-transparent px-5 py-2.5 text-sm font-semibold text-mint transition hover:bg-raised"
              download
            >
            ...
            {hero.socialLinks.map((link) => {
              ...
              return (
                <a
                  ...
                  className="rounded-full border border-border bg-surface p-2.5 text-label-muted transition hover:scale-110 hover:border-mint/60 hover:text-mint"
                >
                  <Icon size={18} />
                </a>
              )
        ...
        >
          <div className="relative grid h-72 w-72 place-content-center rounded-full border border-accent-border bg-raised/70 shadow-[0_0_60px_rgba(192,88,0,0.22)] md:h-80 md:w-80">
            <div className="absolute inset-5 rounded-full border border-mint/40" />
            <div className="orbit-ring absolute inset-[-14px] rounded-full border border-accent-border/70" />
            <div className="animate-float rounded-full border border-border bg-navy px-8 py-8 text-center shadow-glass">
      ...
      <motion.div
        ...
        className="absolute bottom-6 left-1/2 flex -translate-x-1/2 flex-col items-center gap-1 text-xs uppercase tracking-[0.26em] text-label-muted"
      >
